...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda expression: scheme_eval(expression, env))
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 2
        current_env = self
        while current_env.parent and (symbol not in current_env.bindings.keys()):
            current_env = current_env.parent

        if symbol in current_env.bindings.keys():
            return current_env.bindings[symbol]
        else:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is nil:
            return child
        cur_formal = formals.first
        rest_formals = formals.rest
        cur_value = vals.first
        rest_value = vals.rest
        while rest_formals is not nil:
            child.define(cur_formal,cur_value)
            cur_formal = rest_formals.first
            rest_formals = rest_formals.rest
            cur_value = rest_value.first
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        cur = expressions.first
        val = scheme_eval(cur,env)
        rest = expressions.rest
        while (rest is not nil) and is_true_primitive(val):
            cur = rest.first
            rest = rest.rest
            val = scheme_eval(cur,env)
        return val
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        val = scheme_eval(cur,env)
        rest = expressions.rest
        while (rest is not nil) and (is_false_primitive(val)):
            cur = rest.first
            rest = rest.rest
            val = scheme_eval(cur,env)
        return val
...
def make_let_frame(bindings, env):
    ...
    if bindings is not nil:
        ...
        while rest is not nil:
            ...
            values = Pair(eval_all(cur.rest,env),values)
            cur = rest.first
            rest = rest.rest
            validate_form(cur,2,2)
        names = Pair(cur.first,names)
