...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert (self.can_contain(place.ant) or place.ant.can_contain(self)) is True, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dist = 0
        targeted_place = self.place
        # proceed when dist is less than min_range and the place is empty
        while ((targeted_place.bees == []) and (targeted_place!=beehive)) or (dist < self.min_range):
            targeted_place = targeted_place.entrance
            dist += 1
        if targeted_place == beehive or (dist < self.min_range) or (dist > self.max_range):
            return None
        else:
            return random_or_none(targeted_place.bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        addition_damage = 0
        if self.armor <= 0:
            addition_damage = self.damage
        for bee in self.place.bees:
            if amount+addition_damage < bee.armor:
                bee.reduce_armor(amount+addition_damage)
                bee_alive.append(bee)
            else:
                bee.death_callback()
        self.place.bees = bee_alive
        if self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
        # END Problem 5
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        temporary_place = self.place
        if not self.is_true_queen:
            temporary_place = None
            self.reduce_armor(self.armor)
        while temporary_place != None:
            if isinstance(temporary_place.ant,ContainerAnt):
                contained_ant = temporary_place.ant.contained_ant
                if temporary_place.ant and (not temporary_place.doubled):
                    temporary_place.ant.damage *= 2
                if contained_ant and (not contained_ant.doubled):
                    contained_ant.damage *= 2
            else:
                if temporary_place.ant and (not temporary_place.ant.doubled):
                    temporary_place.ant.damage *= 2
    ...
    def remove_from(self, place):
        if isinstance(self,QueenAnt) and (self.is_true_queen):
            pass
        else:
            Ant.remove_from(self,place)
